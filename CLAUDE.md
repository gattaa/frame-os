...
- **haos-addons/frame-uploader/** — a **FastAPI** sidecar, packaged as a Home
  ...
  manifest update — runs inline as a Python function, not a separate
  service) into `data/photos/` + `data/manifest.json`, the only writer of
  both. It's the sole ingest channel today (`channel: "ha"`), but the
  incoming/ contract stays channel-agnostic — a future channel could still
  drop files there, though it would need its own way to get them processed
  since there's no longer an always-on poller watching that directory.
