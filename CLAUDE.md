...
├── haos-addons/    # the processor + the ingest channel, as HAOS add-ons — see below
...
HAOS is the only place the processor and the ingest channel actually run —
there is no separate docker-compose/bare-process deployment to keep in sync.
`haos-addons/frame-pipeline/src/` and `haos-addons/frame-uploader/src/` are
each the single source of truth for that service, edited directly there.
...
- **haos-addons/frame-pipeline/** — the channel-agnostic **processor** (sole
  writer of `photos/` + `manifest.json`), packaged as a Home Assistant OS
  add-on.
- **haos-addons/frame-uploader/** — a **FastAPI** sidecar, also a Home
  ...
  straight from Home Assistant. It is just an ingest channel: it writes only
  into `data/incoming/`. It's the sole ingest channel today (`channel:
  "ha"`), but the contract is channel-agnostic — nothing stops a future
  channel being added without touching this one.
