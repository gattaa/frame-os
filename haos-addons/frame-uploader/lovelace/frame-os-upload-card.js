 ...
 *
 * Lovelace config — choose ONE of the two transport options:
 *
 *   Option A — HA Ingress (preferred; the frame-uploader add-on ships with
 *   `ingress: true` and no port opened on the host):
 *     type: custom:frame-os-upload-card
 *     ingress_slug: frame_uploader          # the installed add-on's slug —
 *                                            # verify yours (Settings > Add-ons
 *                                            # > frame-os Uploader > Info tab);
 *                                            # some Supervisor versions prefix
 *                                            # local add-on slugs with "local_".
 *     token: "<same value as the add-on's upload_token option>"
 *     title: Add a photo                     # optional
 *
 *   Option B — direct URL + shared secret (fallback: use this if you disabled
 *   ingress in the add-on's config.yaml and mapped a port instead, e.g. behind
 ...
 *     title: Add a photo
 *
 * NOTE ON INGRESS: this card resolves the add-on's current ingress URL via
 * `hass.callApi('get', 'hassio/addons/<slug>/info')` -> `.data.ingress_url`,
 * per Home Assistant's documented Supervisor API
 * (https://developers.home-assistant.io/docs/api/supervisor/endpoints#addons).
 * That call requires the logged-in HA user to be an administrator (a
 * Supervisor API restriction, not specific to this card) — non-admin family
 * members should use a dashboard where this card is pre-configured, or Option
 * B. This path follows documented add-on conventions but was not exercised
 * against a live Home Assistant Supervisor while building this project (no
 * HAOS instance was available); verify it on your own install. Option B is a
 * plain fetch with no such dependency.
 */

class FrameOsUploadCard extends HTMLElement {
  setConfig(config) {
    if (!config || (!config.sidecar_url && !config.ingress_slug)) {
      throw new Error("frame-os-upload-card: set either 'ingress_slug' or 'sidecar_url'");
    }
    this._config = config;
    this._built = false;
  }
  ...
  // Resolve fresh on each submit (rather than caching) so a rotated ingress
  // token after an add-on restart never leaves the card pointed at a stale URL.
  async _resolveUploadUrl() {
    if (this._config.ingress_slug) {
      const resp = await this._hass.callApi("get", `hassio/addons/${this._config.ingress_slug}/info`);
      const base = resp && resp.data && resp.data.ingress_url;
      if (!base) {
        throw new Error("no ingress_url for '" + this._config.ingress_slug + "' — is the add-on installed and ingress enabled?");
      }
      return base.replace(/\/$/, "") + "/upload";
    }
    return this._config.sidecar_url;
  }
  ...
  async _onSubmit() {
    ...
    let url;
    try {
      url = await this._resolveUploadUrl();
    } catch (err) {
      this._setStatus("Could not resolve upload URL: " + err.message, "var(--error-color, #c00)");
      return;
    }
