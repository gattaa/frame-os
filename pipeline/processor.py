...
MAX_LONG_EDGE = 1280      # downscale so the long edge is at most this many px
JPEG_QUALITY = 85         # re-encode quality
...
POLL_INTERVAL = 30.0      # --loop poll cadency (seconds)
...
class Config:
    ...
    def from_args(cls, args: argparse.Namespace) -> "Config":
        ...
        data = root / "data"
        incoming = Path(args.incoming) if args.incoming else data / "incoming"
        photos = Path(args.photos) if args.photos else data / "photos"
        manifest = Path(args.manifest) if args.manifest else data / "manifest.json"
